...
par(mfcol=c(2,2))
with(data2,{
  plot(data2$Global_active_power ~ data2$DayFormat, xlab = "", ylab = "Global Active Power", type = "n")
  ...
  plot(data2$Voltage ~ data2$DayFormat, xlab = "datetime", ylab = "Voltage", type = "n")
  lines(data2$Voltage ~ data2$DayFormat)
  
  plot(data2$Global_reactive_power ~ data2$DayFormat, xlab = "datetime", ylab = "Global_reactive_power", type = "n")
  lines(data2$Global_reactive_power ~ data2$DayFormat)
})
dev.off()